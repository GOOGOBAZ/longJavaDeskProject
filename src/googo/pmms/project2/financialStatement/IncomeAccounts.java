...
otherIncomeAccounts.add("Income From Short Term Insurance Business");
  ...
  public List<List<Object>> mainIncome(){
...
for(Map.Entry<Integer, String> entry:level5.entrySet()){
        ...
        if(parseDouble(rdb.getBalances(entry.getValue(),incomeDateStart,incomeDateEnd).get(entry.getValue()).toString())!=0.0){
        ...
        mainIncome.add(parseDouble(rdb.getBalances(entry.getValue(),incomeDateStart,incomeDateEnd).get(entry.getValue()).toString()));
        


        if(parseDouble(rdb.getBalances(entry.getValue(),ffm.earlierDate(incomeDateStart),ffm.earlierDate(incomeDateEnd)).get(entry.getValue()).toString())==0){
        
            mainIncome.add("-");
            
        }else{
        
        mainIncome.add(parseDouble(rdb.getBalances(entry.getValue(),ffm.earlierDate(incomeDateStart),ffm.earlierDate(incomeDateEnd)).get(entry.getValue()).toString()));
...
mainIncomeListList.add(mainIncome);
    ...
    public List<List<Object>>  otherIncomes(){
...
for(Map.Entry<Integer, String> entry:level5.entrySet()){
        ...
        otherIncomeK.add(parseDouble(rdb.getBalances(entry.getValue(),incomeDateStart,incomeDateEnd).get(entry.getValue()).toString()));
        ...
        otherIncomeK.add(rdb.getBalances(entry.getValue(),ffm.earlierDate(incomeDateStart),ffm.earlierDate(incomeDateEnd)).get(entry.getValue()).toString());
