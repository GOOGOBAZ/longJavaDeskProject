...
equityT.add("Share Capital");
 equityT.add("Reserves And Surplus");   
  ...
  public List<List<Object>>  currentLiabilities(){
...
//       fios.stringFileWriter(fios.createFileName("test", "testit", "trdyr.txt"),balanceSheetDate); 
        ...
        if(parseDouble(rdb.getBalancesSheet(entry.getValue(),balanceSheetDate).get(entry.getValue()).toString())!=0.0){
        ...
        currentLiabilities.add(parseDouble(rdb.getBalancesSheet(entry.getValue(),balanceSheetDate).get(entry.getValue()).toString()));
        
        if(parseDouble(rdb.getBalancesSheet(entry.getValue(),ffm.earlierDate(balanceSheetDate)).get(entry.getValue()).toString())==0){
        
            currentLiabilities.add("-");
            
        }else{
        
        currentLiabilities.add(parseDouble(rdb.getBalancesSheet(entry.getValue(),ffm.earlierDate(balanceSheetDate)).get(entry.getValue()).toString()));
        }
    ...
    public List<List<Object>>  nonCurrentLiabilities(){
...
for(Map.Entry<Integer, String> entry:level5.entrySet()){
       ...
       nonCurrentLiabilities.add(parseDouble(rdb.getBalancesSheet(entry.getValue(),balanceSheetDate).get(entry.getValue()).toString()));
       ...
       nonCurrentLiabilities.add(parseDouble(rdb.getBalancesSheet(entry.getValue(),ffm.earlierDate(balanceSheetDate)).get(entry.getValue()).toString()));
        }
...
public List<Object> equityTitle(){
  ...
  public List<List<Object>>  equity(){
 ...
 List<List<Object>> equityv =new ArrayList();
        ...
        balanceSheetCurrentAmount=parseDouble(rdb.getBalancesSheet(entry.getValue(),balanceSheetDate).get(entry.getValue()).toString());

        balanceSheetEarlierAmount=parseDouble(rdb.getBalancesSheet(entry.getValue(),ffm.earlierDate(balanceSheetDate)).get(entry.getValue()).toString());

        accountName=  entry.getValue();  

        currentProfits=  parseDouble(netprofits.theNetProfits(ffm.forDatabaseWithFullYearBeginningWithDate(fios.stringFileReader(fios.createFileName("persistence", "operatingCycle", "YEARstartDate.txt"))),balanceSheetDate));

        earklierProfits= parseDouble(netprofits.theNetProfits(ffm.earlierDate(ffm.forDatabaseWithFullYearBeginningWithDate(fios.stringFileReader(fios.createFileName("persistence", "operatingCycle", "YEARstartDate.txt")))),ffm.earlierDate(balanceSheetDate)));
