...
//        fios.stringFileWriter(fios.createFileName("test", "testit", accountNumber+"trdyr.txt"),BatchNumber);
        ...
        public synchronized boolean loanRepaymentsNew(List debit ,List  credit,List processOrder,List otherOrders){
...
newPaymentCame.add("0.0");//LoanPenaltyPaid
  ...
  SwingWorker<Void,Void> createTheInitailLoanPayState=new SwingWorker(){
        ...
        currentLoanPenaltyNow=loan.currentLoanPenaltytNow(accountNumberNow,currentInstalmentId);//Pick the loan accumulatedInterest instalment from the schedule that is pending   
...
//        protected Object doInBackground() throws Exception {   
        ...
        processInterestPayment(totalInterestPaid,credit,fmt.formatAccountWithSeperators(debit.get(0).toString()));   
...
//        @Override
//        protected Object doInBackground() throws Exception {   
        processLoanPenaltyPayment(totalLoanPenaltyPaid,credit,fmt.formatAccountWithSeperators(debit.get(0).toString()));
//        return null;
//        }  };
//        payTotalLoanPenaltyPaid.execute();
        successful2=true;
...
//        @Override
//        protected Object doInBackground() throws Exception {   
        processPrincipalPayment(totalPrincipalPaid,credit,fmt.formatAccountWithSeperators(debit.get(0).toString())); 
//        return null;
//        }  };
//        payTotalPrincipalPaid.execute();   
        ...
        public synchronized void updateLoanBalances(List debit){
...
public synchronized boolean loanRepaymentsNewBank(List debit ,List  credit,List processOrder,List otherOrders){
...
//        }
        ...
        sendsms.createDisbursementPaymentSMS(fmt.formatAccountWithSeperators(credit.get(0).toString()), credit.get(5).toString());
...
public boolean loanDisbursementBank(List debit ,List credit,String txnCode){
 ...
 dbq.reduceSavings(debit);
        ...
        sendsms.createSharesPaymentSMS(fmt.formatAccountWithSeperators(debit.get(0).toString()), debit.get(5).toString());
...
public boolean regularShareContributionBank(List debit ,List credit,String txnId){
 ...
 dbq.reduceSavings(debit);
        ...
        return confirm;
           ...
           public boolean generalPosting1xx(List debit ,List credit){
        ...
        if(dbq.debit1xx(debit,c)){



        confirm1= true;
        if(dbq.credit1xx(credit,c)){
        confirm2= true;
        }
        ...
        sendsms.createSavingsSMS(fmt.formatAccountWithSeperators(credit.get(0).toString()), credit.get(5).toString());
        }
...
//dbq.initialiseSharesValueTotal();
        ...
        sendsms.createwithdrawTransacttSMS(fmt.formatAccountWithSeperators(debit.get(0).toString()), debit.get(5).toString());
