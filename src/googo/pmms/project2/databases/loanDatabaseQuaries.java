...
//    
   ...
   fios.stringFileWriter(fios.createFileName("loanApplication", "loanProcessing", "accrueingStatus"+accountNumber+".txt"),"NeverAccrued");   
    ...
    public synchronized void insertLoanAppStore(ArrayList value,List value1,Component c){
//JOptionPane.showMessageDialog(c, "Inside"+value1.size());
    ...
    ps.setObject(2, dbq.getDBCurrentDate());// trn_date    :2
  ...
  public ObjectTableModel loanManagementTableFilltWrittenOffLoans(JTable table, ArrayList<String> headers){
   ...
   String query = "SELECT trn_id,loan_id,applicant_account_name,princimpal_amount, TotalPrincipalRemaining,TotalInterestRemaining,TotalAccumulatedInterestRemaining,TotalLoanPenaltyRemaining,balance_due,loan_cycle_status,trn_date,gruop_id FROM  new_loan_appstore WHERE loan_cycle_status="+"'"+"WrittenOff"+"'"+" ORDER BY applicant_account_name ASC";
    ...
    return model;
     ...
     public loanManagementModel loanManagementTableFilltActiveLoans(JTable table, ArrayList<String> headers){
loanManagementModel    modelM=null;
    try {
List    data5M= new ArrayList<>();
    Connection cq=loancon.createConnection(); 
    cq.setAutoCommit(false);
   String query = "SELECT trn_id,"
           + "loan_id,"
           + "applicant_account_name,"
           + "princimpal_amount, "
           + "TotalPrincipalRemaining,"
           + "TotalInterestRemaining,"
           + "TotalAccumulatedInterestRemaining,"
           + "TotalLoanPenaltyRemaining,"
           + "balance_due,"
           + "loan_cycle_status,"
           + "trn_date,gruop_id FROM  new_loan_appstore WHERE (loan_cycle_status="+"'"+"Disbursed"+"'"+" OR loan_cycle_status="+"'"+"Applied"+"'"+" OR loan_cycle_status="+"'"+"Approved"+"')  ORDER BY applicant_account_name ASC";
    PreparedStatement ps = cq.prepareStatement(query);
    ResultSet rst3 = ps.executeQuery();
    int k=0;
    while (rst3.next()){
 List   data4m    =new ArrayList();
    ...
    data4m.add(1, rst3.getString("loan_id"));
     data4m.add(2, rst3.getString("applicant_account_name"));
    data4m.add(3, df2.format(rst3.getDouble("princimpal_amount")));
     data4m.add(4, df2.format(rst3.getDouble("TotalPrincipalRemaining")));
    data4m.add(5, df2.format(rst3.getDouble("TotalInterestRemaining")));
      data4m.add(6, df2.format(rst3.getDouble("TotalAccumulatedInterestRemaining")));
    data4m.add(7, df2.format(rst3.getDouble("TotalLoanPenaltyRemaining")));
    data4m.add(8, df2.format(rst3.getDouble("balance_due")));
    data4m.add(9, rst3.getString("loan_cycle_status"));
...
modelM= new loanManagementModel( data5M, headers);
    ...
    Connection cq=loancon.createConnection(); 
    cq.setAutoCommit(false);
   String query = "SELECT trn_id,loan_id,applicant_account_name,princimpal_amount, TotalPrincipalRemaining,TotalInterestRemaining,TotalAccumulatedInterestRemaining,TotalLoanPenaltyRemaining,balance_due,loan_cycle_status,trn_date,gruop_id FROM  new_loan_appstore WHERE loan_cycle_status="+"'"+"Completed"+"'" +"ORDER BY applicant_account_name ASC";
    PreparedStatement ps = cq.prepareStatement(query);
    ResultSet rst3 = ps.executeQuery();
    int k=0;
    while (rst3.next()){
...
String instalmentPaid="0.0";
  ...
  List<List>  data5= new ArrayList<>();
    Connection cq=loancon.createConnection(); 
    cq.setAutoCommit(false);
    String query = "SELECT * FROM new_loan_appstoreamort WHERE master2_id="+"'"+closedLoanAccount+"'";
    PreparedStatement ps =cq.prepareStatement(query);
    ResultSet rst3 = ps.executeQuery();
    int k=0;
    while (rst3.next()){
...
public void loanSecurity(JTable table1,String loanId){
String loanIDsE="";
 ...
 Connection cq=loancon.createConnection();
   cq.setAutoCommit(false);
            String query = "SELECT  SecurityLoan FROM new_loan_appstore WHERE loan_id="+"'"+loanId+"'";
...
return theLoanPinstalmentNow;
        ...
        public double currentLoanPenaltytNow(String accountNumber, int trnId){
...
return theTotalPrincipalPaid;
        ...
        private synchronized void updateAllRepaymentsReports(String type,String accountNumberNow,int currentInstalmentId,double amount){
...
String theProcedure ="CALL DateManagementForLenders("+"'"+interestPaid+"'"+","+"'newloan"+accountNumberNow+"'"+")";
 ...
 cq.setAutoCommit(true);
      ...
      public synchronized void updateInterestRepaymentsInterest(){
        
         try {
                Connection cq=loancon.createConnection(); 
          cq.setAutoCommit(false);
String theProcedure ="CALL InterestManagementForLenders()";
...
public synchronized void updateNewInterestNow(String accountNumberNow,int currentInstalmentId,double currentInterestNow,Component c){
   ...
   SwingWorker<Void,Void> updateAllRepaymentsReports=new SwingWorker(){
                        ...
                        updateAllRepaymentsReports("updateNewInterestNow",accountNumberNow,currentInstalmentId,currentInterestNow); 
                                return null;
                            }  };
...
public synchronized void updateNewAccumulatedInterestNow(String accountNumberNow,int currentInstalmentId,double currentAccumulatedInterestNow,Component c){
     ...
     SwingWorker<Void,Void> updateAllRepaymentsReports=new SwingWorker(){
                            @Override
                            protected Object doInBackground() throws Exception {   
                        updateAllRepaymentsReports("updateNewAccumulatedInterestNow",accountNumberNow,currentInstalmentId,currentAccumulatedInterestNow); 
                                return null;
                            }  };
                     updateAllRepaymentsReports.execute();
...
public synchronized void updateNewLoanPenaltyNow(String accountNumberNow,int currentInstalmentId,double currentPenaltyNow,Component c){
       ...
       SwingWorker<Void,Void> updateAllRepaymentsReports=new SwingWorker(){
                            @Override
                            protected Object doInBackground() throws Exception {   
                        updateAllRepaymentsReports("updateNewLoanPenaltyNow",accountNumberNow,currentInstalmentId,currentPenaltyNow); 
                                return null;
                            }  };
                     updateAllRepaymentsReports.execute();
...
public synchronized void updateNewLoanPrincipalNow(String accountNumberNow,int currentInstalmentId,double currentPrincipalNow,Component c){
       ...
       SwingWorker<Void,Void> updateAllRepaymentsReports=new SwingWorker(){
                            @Override
                            protected Object doInBackground() throws Exception {   
                        updateAllRepaymentsReports("updateNewLoanPrincipalNow",accountNumberNow,currentInstalmentId,currentPrincipalNow); 
                                return null;
                            }  };
                     updateAllRepaymentsReports.execute();
...
public synchronized void createLoanPenalty(String accountNumberx,double amount,int instalmentNumber){
...
double loanPenaltyRemaining=currentLoanPenaltytNow(accountNumber, instalmentNumber);
